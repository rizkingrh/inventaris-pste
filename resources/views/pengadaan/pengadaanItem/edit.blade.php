    ...
    <div id="edit-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form action="{{ url('pengadaan-item/' . $item->id) }}" method="POST" class="p-4 md:p-5">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div class="sm:col-span-2" style="display: none;">
                            <input type="hidden" name="pengadaan_id" id="pengadaan_id" value="{{ $data->id }}">
                        </div>
                        <div>
                            ...
                            <input type="number" name="harga_beli" id="harga_beli"
                                ...
                                placeholder="Masukan harga beli" required="" value="{{ $item->harga_beli }}">
                        </div>
                        <div>
                            <label for="jumlah"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah</label>
                            <input type="number" name="jumlah" id="jumlah" max="9999999999"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <select id="barang_id" name="barang_id"
                                ...
                                <option hidden value="{{ $item->barang_id }}">{{ $item->barang->kode_barang }} -
                                    {{ $item->barang->nama_barang }}</option>
                                @foreach ($barang as $brg)
                                    <option value="{{ $brg->id }}">
                                        {{ $brg->kode_barang }} - {{ $brg->nama_barang }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="sm:col-span-2">
