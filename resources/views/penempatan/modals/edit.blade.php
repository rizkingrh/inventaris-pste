    ...
    <div id="edit-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form action="{{ url('penempatan/' . $item->id) }}" method="POST" class="p-4 md:p-5">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="nomor_penempatan" id="nomor_penempatan"
                                ...
                                placeholder="PNT000" required="" value="{{ $item->nomor_penempatan }}" disabled
                                readonly>
                        </div>
                        <div>
                            ...
                            <div class="relative max-w-sm">
                                ...
                                <input datepicker datepicker-buttons datepicker-autoselect-today type="text"
                                    ...
                                    placeholder="Pilih tanggal" value="{{ $item->tanggal_penempatan }}">
                            </div>
                        </div>
                        <div>
                            ...
                            <select id="ruangan_id" name="ruangan_id"
                                ...
                                <option hidden value="{{ $item->ruangan_id }}">
                        ...
                        <div>
                            ...
                            <select id="user_id" name="user_id"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                <option hidden value="{{ $item->user_id }}">{{ $item->user->name }}
                                </option>
                                @foreach ($user as $us)
                                    <option value="{{ $us->id }}">
                                        {{ $us->name }}</option>
