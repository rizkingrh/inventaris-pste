        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form action="{{ url('mutasi') }}" method="POST">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <label for="nomor_mutasi"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomor
                            Mutasi</label>
                        <input type="text" name="nomor_mutasi" id="nomor_mutasi"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="MTS000" required="" value="{{ old('nomor_mutasi') }}">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="user_id" name="user_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option selected disabled hidden>Pilih petugas/user</option>
                            @foreach ($user as $us)
                                <option value="{{ $us->id }}">
                                    {{ $us->name }}</option>
                            @endforeach
                    ...
                    <div class="sm:col-span-2">
                        <label for="penempatan_id"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Penempatan</label>
                        <select id="penempatan_id" name="penempatan_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option selected disabled hidden>Pilih penempatan</option>
                            @foreach ($penempatan as $pnt)
                                <option value="{{ $pnt->id }}">
                                    {{ $pnt->nomor_penempatan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="keterangan" name="keterangan" rows="2"
                            ...
                            placeholder="Masukan keterangan" required>{{ old('keterangan') }}</textarea>
                    </div>
                </div>
                <button type="submit"
